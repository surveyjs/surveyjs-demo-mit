import { LEADS_USERS, type SessionUser } from "@/schemas";

export type { SessionUser };

// The Leads users are declared once, in `src/schemas/variables/leads.ts`, beside
// the variable presets built from the same list.
export { LEADS_USERS };
 ...
 *
 * A records page reads this on the server and publishes the first user to the
 * form as variables, one per field (`toVariables`), so the definition can read
 * `{user_role}`. In your app
