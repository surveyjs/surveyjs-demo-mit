...
| `src/lib` | Helpers with no React: route builders, the CMS-1500 printer, the survey-core linter adapter, the (dormant here) license-key loader. |
...
**`NEXTJS_PUBLIC_SLK` — the SurveyJS commercial license key.** It reaches the browser, forwarded by the `env` block in `next.config.mjs` rather than by a `NEXT_PUBLIC_` prefix, so the two must be renamed together. This edition does not use it at all, because nothing here is licensed. The file that applies it, `src/lib/surveyjs-license.ts`, ships here anyway and stays dormant: `slk` is exported by `survey-core` itself, so the code is MIT-clean, and the two editions differ by a key rather than by code.
