...
| `src/components` | React. `AdminShell`, `TopBar` and `Sidebar` are the chrome, `how-built/` the "How this page is built" toggle's state and the panel it opens, `SurveyForm` the one way a survey is rendered in the shell, `records/` the shared records page (`RecordsView`, and the header's user switcher), `ClaimsView` a thin wrapper over `RecordsView` that adds extraction and the CMS-1500 export, `NotImplemented` the panel of a page not built yet (unused since `/leads` was built), `configure/` the editor (also rendered inside the shell on `/definition`), `embedded/` the three host sites and their shared toolbar, `ui/` the shadcn primitives. |
...
| `e2e` | Playwright. `initial.spec.ts` walks every route, `records.spec.ts` covers the shared records page and the how-built panel on `/claims`, `leads.spec.ts` the CRM record on `/leads` (totals, the economic-buyer rule, roles, row ids), the others cover the editor, the linter (its front end and `/api/lint`) and the extractor. |
...
6. **The page**, about fifteen lines: read `listResults(id)`, `getResult` for the first row and, if the page has users, `listSessionUsers(id)` on the server, and render `RecordsView` with them. `RecordsView` renders the page header too. `claims/page.tsx` and `ClaimsView.tsx` show the optional props: `exportPdf` to replace the generic PDF export, `listFooter` under the list, `formNote` under the form's heading. `leads/page.tsx` shows `users` and `layout="stacked"`, which puts the list above a full-width form: beside the list the form is narrower than the shadcn adapter's `--sd-mobile-width` (640px), and every matrix turns into stacked cards.
